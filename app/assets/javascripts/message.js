$(function() {
  let buildHTML = message => {
    let messageInfo = `<div class="message__info">
                         <p class="message__info__user-name">${
                           message.user_name
                         }<p>
                         <p class="message__info__daytime">${
                           message.created_at
                         }<p>
                       </div>`;
    let messageText = message.content
      ...
      : "";
    let messageImage = message.image.url
      ? `<img src="${message.image.url}" width="200" height="200">`
      : "";
    let html = `<div class="message" data-group-id="${
      message.group_id
  ...
  $("#new_message").on("submit", function(e) {
    ...
    $.ajax({
      ...
      .done(function(message) {
        ...
        $(".messages")
          .append(html)
          .animate({ scrollTop: $(".messages")[0].scrollHeight }, "fast");
