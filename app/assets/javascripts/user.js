$(function() {
  ...
  let group_user_ids = $.makeArray(
    group_users.children().map(function(i, value) {
  ...
  let appendSearchUser = user => {
    let html = `<div class="chat-group-user clearfix">
                  ...
                  <div class="user-search-add chat-group-user__btn chat-group-user__btn--add" data-user-id="${
                    user.id
                  }" data-user-name="${user.name}">追加</div>
                </div>`;
  ...
  let appendGroupMember = member => {
    let html = `<div class='chat-group-user clearfix js-chat-member' id='chat-group-user-${
      member.userId
